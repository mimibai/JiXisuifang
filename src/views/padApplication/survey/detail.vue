      ...
      >
        <div class="survey">
          <div
            ...
            v-loading="loading"
          >
            <div class="caseJoinList-hospitalInfo">
              <div class="back-list">
                <el-button
                  type="success"
                  size="small"
                ...
                <div class="survey-main-content-btn">
                  <el-button-group>
                    ...
                    <el-button
                      ...
                      :disabled="isLocked"
                      @click="updateTerminalFinishState"
...
import {
  getTerminalCaseProgressPeriodList,
  getTerminalConsent,
  updateTerminalConsentState,
  // getTerminalMINIPage,
  getTerminalFollowupMINIPageList,
  // terminaltopicSub,
  updateTerminalFinishState,
  lockTerminalCaseJoinProtocol,
  // checkIsExistNotSucProtocal,
  getTerminalNewMINIPage,
  generateTerminalFillTopic,
  getTerminalFollowupMINITriggerEventsByTopicCode,
  terminalFilledPageSub
} from '@/api/summary'
...
export default {
  ...
  methods: {
    ...
    // 获取就诊人当前进行阶段
    getTerminalCaseProgressPeriodList(param) {
      getTerminalCaseProgressPeriodList(param)
        .then((response) => {
          const { Data } = response
          if (Data !== null) {
            ...
            for (let index = 0; index < Data.length; index++) {
              ...
              if (item.followupProjectTypeId === 1) {
                ...
                this.caseJoinProtocolId = item.caseJoinProtocolId
                this.caseProgressPeriodId = item.caseProgressPeriodId
                this.getTerminalFollowupMINIPageList(item)
    ...
    // 弹窗读取知情同意书
    getTerminalConsent(param) {
      // 获取知情同意书内容
      getTerminalConsent(param).then((response) => {
        const { Data } = response
    ...
    handleClose(param) {
      if (this.dialogChecked && param) {
        updateTerminalConsentState({
          ...
          // 获取就诊人参与方案
          this.getTerminalFollowupMINIPageList(transferMsg)
    ...
    getPageData(miniPageId) {
      ...
      getTerminalNewMINIPage(param).then((response) => {
        const { Data } = response
        const { data, pageName, isLocked } = Data
    ...
    getEventsCode(getEventParam) {
      getTerminalFollowupMINITriggerEventsByTopicCode(getEventParam).then(
        (res) => {
          const { Data } = res
    ...
    postGenerateFillTopic(params) {
      generateTerminalFillTopic(params).then((res) => {})
    },
    // form表单事件
    ...
    async submitMinPage() {
      ...
      await terminalFilledPageSub(formSubmitData).then((res) => {})
    },
    // 修改问卷完成标记
    updateTerminalFinishState() {
      this.$refs.dynamicValidateForm.validate((valid, object) => {
        if (valid) {
          this.submitMinPage().then((res) => {
            ...
            updateTerminalFinishState(param).then((response) => {
              MessageBox.confirm(
                '确定提交后将无法更改，是否确定提交？',
                '确定提交',
                {
                ...
                // 锁定问卷
                lockTerminalCaseJoinProtocol(param).then((res) => {
                  this.$router.push({
